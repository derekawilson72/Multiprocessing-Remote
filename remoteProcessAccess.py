...
class remoteProc():
    ...
    running in the background as its own process.  The main process can access the multiprocessing variables of num, arr and keepLooping.  Users can control the loop running in a thread by setting the value to 0.  
    ...
    def __init__(self,
        ...
        """
        Instantiate the class setting the default variables.  Set  keepLooping to 0 to kill the loops.
        """
        self.num=num
        self.arr=arr
    ...
    def doComplicatedStuff(self):
        """
        The arbitrary function or procedure to run in the background.  import anything here and control the loop by setting the keepLooping value to 0.  Also watch the num variable increment by one each time.
        """
        import time
    ...
    def startThread(self):
        """
        Run the function as a thread to allow users to interact with the class parameters independantly in Ipython.
        """
...
def launchIPythonProcess(num,arr,keepLooping):
    """
    Call the remoteProc function and start the thread.  Finally call Ipython to run the class functions in a thread in the background
    """
    import IPython
